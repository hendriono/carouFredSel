...
(function($) {
	...
	$.fn.carouFredSel = function(options, configs) {
		...
		$cfs.init = function(o, setOrig, start) {
			...
			opts.auto		= go_getNaviObject($tt0, opts.auto, false, true);
			opts.prev		= go_getNaviObject($tt0, opts.prev);
			opts.next		= go_getNaviObject($tt0, opts.next);
			opts.pagination	= go_getNaviObject($tt0, opts.pagination, true);
		...
		$cfs.bind_events = function() {
			...
			//	stop event
			$cfs.bind('stop.cfs'+serial, function(e, imme) {
				e.stopPropagation();
				crsl.isStopped = true;
				if (opts.auto.play) {
					opts.auto.play = false;
					$cfs.trigger('pause', imme);
				}
			});
			...
			//	pause event
			$cfs.bind('pause.cfs'+serial, function(e, imme) {
				e.stopPropagation();
				tmrs = sc_clearTimers(tmrs);

				//	immediately pause
				if (imme && crsl.isScrolling) {
					scrl.isStopped = true;
				...
				if (!crsl.isPaused && !crsl.isScrolling) {
					if (opts.auto.pauseOnHover.indexOf('resume') > -1) {
			...
			$cfs.bind('slide_prev.cfs'+serial, function(e, sO, nI) {
				...
				var a_complete = function() {
					...
					var fn = function() {
						...
						if (sO.onAfter) sO.onAfter.call($tt0, args);
			...
			$cfs.bind('currentPage.cfs'+serial, function(e, fn) {
				e.stopPropagation();
				var max = Math.ceil(itms.total/opts.items.visible-1);
				if (itms.first == 0) 											var nr = 0;
				else if (itms.first < itms.total % opts.items.visible) 			var nr = 0;
				else if (itms.first == opts.items.visible && !opts.circular) 	var nr = max;
				else 															var nr = Math.round((itms.total-itms.first)/opts.items.visible);
				if (nr < 0) nr = 0;
				if (nr > max) nr = max;
			...
			$cfs.bind('updatePageStatus.cfs'+serial, function(e, build, sizes) {
				...
				if (typeof build == 'boolean' && build) {
					...
					for (var a = 0, l = Math.ceil(itms.total/opts.items.visible); a < l; a++) {
						var i = $cfs.children().eq( gn_getItemIndex(a*opts.items.visible, 0, true, itms, $cfs) );
				...
				opts.pagination.container.children().removeClass('selected').eq($cfs.triggerHandler('currentPage')).addClass('selected');
			});
		...
		$cfs.bind_buttons = function() {
			...
			if (opts.auto.pauseOnHover) {
				...
				$wrp.bind('mouseenter.cfs'+serial, function() { $cfs.trigger('pause', pC[0]);	})
					.bind('mouseleave.cfs'+serial, function() { $cfs.trigger(pC[1]);			});
			}
			...
			if (opts.prev.button) {
				...
				if (opts.prev.pauseOnHover) {
					...
					opts.prev.button.bind('mouseenter.cfs'+serial, function() { $cfs.trigger('pause', pC[0]);	})
									.bind('mouseleave.cfs'+serial, function() { $cfs.trigger(pC[1]);			});
				}
			}
			...
			if (opts.next.button) {
				...
				if (opts.next.pauseOnHover) {
					...
					opts.next.button.bind('mouseenter.cfs'+serial, function() { $cfs.trigger('pause', pC[0]); 	})
									.bind('mouseleave.cfs'+serial, function() { $cfs.trigger(pC[1]);			});
				}
			}
			if ($.fn.mousewheel) {
			...
			if (opts.pagination.container) {
				if (opts.pagination.pauseOnHover) {
					...
					opts.pagination.container.bind('mouseenter.cfs'+serial, function() { $cfs.trigger('pause', pC[0]);	})
											 .bind('mouseleave.cfs'+serial, function() { $cfs.trigger(pC[1]);			});
				}
			}
...
//			$cfs.trigger('slideTo', [s, 0, true, { duration: 0 }, 'next']);
	...
	function go_getNaviObject($tt, obj, pagi, auto) {
	...
	//	buttons functions
	function bt_pauseOnHoverConfig(poh) {
		var g = (poh.indexOf('immediate') > -1) ? true : null;
		return [g, 'resume'];
	}
